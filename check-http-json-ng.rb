...
class CheckJson < Sensu::Plugin::Check::CLI
  ...
  option :redirect_limit, :short => '-R VALUE', :long => '--redirect-limit VALUE', :default => 10
  ...
  def run
    begin
      timeout(config[:timeout]) do
        get_resource(config[:url], config[:redirect_limit].to_i)
      end
